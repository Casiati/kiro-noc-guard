...
if ($KB_BUCKET) {
    ...
    if ($LANG_CHOICE -ne "2") {
        $env:KB_DIRECTIVE_TEXT = "## Initial Checklist - Knowledge Base (BEFORE INVESTIGATING)``n``nAs soon as you receive a new alert/incident (not a one-off single-command query), BEFORE starting the AWS/K8s investigation:``n``n1. Does the alert identify a specific resource/service (e.g., ECS service name, cluster, task definition, instance, named alarm) - not a generic single-command question?``n2. If YES to #1: your FIRST action, before any other investigation command, MUST be searching the Knowledge Base for that resource/alert type: ``python3 ~/.kiro/noc-guard/skills/knowledge-builder/kb_manager.py --bucket $KB_BUCKET$KB_PROFILE_FLAG --action search --query `"<service/resource name or alert type>`"``.``n3. If the search returns a relevant, recent record: do NOT repeat the investigation from scratch. Summarize the prior finding to the operator, state the record's date, and only re-investigate the specific points that may have changed since then (e.g., a new spike, different behavior than what was already documented) - the goal is avoiding duplicate analyses, not skipping validation whenever there's genuine doubt.``n4. If there is no relevant record, or the search returns nothing: proceed normally with the full investigation (and, at the end, follow the Final Checklist below).``n5. If it's a one-off/trivial single-command query (not an incident investigation): skip this step.``n``n## Final Checklist - Knowledge Base (NEXT)``n``nBefore ending EVERY response where you investigated an AWS/K8s/infra incident, check:``n``n1. Did this investigation involve 3+ read commands (AWS/logs/metrics/kubectl) AND did you reach an explicit conclusion (root cause identified, OR confirmed false positive, OR clear recommendation)?``n2. If YES to #1: your NEXT action, before finishing, MUST be running ``python3 ~/.kiro/noc-guard/skills/knowledge-builder/kb_manager.py --bucket $KB_BUCKET$KB_PROFILE_FLAG --action add --alert `"<short alert/topic name>`" --content `"<concise summary: root cause, evidence, recommendation>`"``. Do this silently as part of your workflow, then mention in your final answer that the finding was recorded.``n3. If NO (quick lookup, single command, no conclusion reached): skip this step, no need to mention it.``n``nTo search the Knowledge Base for prior similar incidents, use: ``python3 ~/.kiro/noc-guard/skills/knowledge-builder/kb_manager.py --bucket $KB_BUCKET$KB_PROFILE_FLAG --action search --query `"<term>`"``."
    } else {
        $env:KB_DIRECTIVE_TEXT = "## Checklist Inicial - Base de Conhecimento (ANTES DE INVESTIGAR)``n``nAssim que receber um novo alerta/incidente (nao uma consulta pontual de comando unico), ANTES de iniciar a investigacao AWS/K8s:``n``n1. O alerta identifica um recurso/servico especifico (ex: nome de servico ECS, cluster, task definition, instancia, alarme nomeado) - nao e uma pergunta generica de unico comando?``n2. Se SIM ao item 1: sua PRIMEIRA acao, antes de qualquer outro comando de investigacao, DEVE ser buscar na Base de Conhecimento por esse recurso/tipo de alerta: ``python3 ~/.kiro/noc-guard/skills/knowledge-builder/kb_manager.py --bucket $KB_BUCKET$KB_PROFILE_FLAG --action search --query `"<nome do servico/recurso ou tipo de alerta>`"``.``n3. Se a busca retornar um registro relevante e recente: NAO repita a investigacao do zero. Resuma o achado anterior ao operador, informe a data do registro, e so investigue novamente os pontos especificos que possam ter mudado desde entao (ex: novo pico, comportamento diferente do ja documentado) - o objetivo e evitar duplicar analises ja feitas, nao pular a validacao sempre que houver duvida real.``n4. Se NAO houver registro relevante, ou a busca nao retornar nada: prossiga normalmente com a investigacao completa (e, ao final, siga o Checklist Final abaixo).``n5. Se for uma consulta pontual/trivial de unico comando (sem caracterizar investigacao de incidente): pule esta etapa.``n``n## Checklist Final - Base de Conhecimento (PROXIMO PASSO)``n``nAntes de finalizar TODA resposta em que voce investigou um incidente de AWS/K8s/infra, verifique:``n``n1. Essa investigacao envolveu 3 ou mais comandos de leitura (AWS/logs/metricas/kubectl) E voce chegou a uma conclusao explicita (causa raiz identificada, OU falso positivo confirmado, OU recomendacao clara)?``n2. Se SIM ao item 1: sua PROXIMA acao, antes de terminar, DEVE ser executar ``python3 ~/.kiro/noc-guard/skills/knowledge-builder/kb_manager.py --bucket $KB_BUCKET$KB_PROFILE_FLAG --action add --alert `"<nome curto do alerta/topico>`" --content `"<resumo conciso: causa raiz, evidencias, recomendacao>`"``. Faca isso silenciosamente como parte do seu fluxo, depois mencione na resposta final que o achado foi registrado.``n3. Se NAO (consulta rapida, comando unico, sem conclusao): pule esta etapa, sem necessidade de mencion-la.``n``nPara pesquisar na Base de Conhecimento por incidentes semelhantes anteriores, utilize: ``python3 ~/.kiro/noc-guard/skills/knowledge-builder/kb_manager.py --bucket $KB_BUCKET$KB_PROFILE_FLAG --action search --query `"<termo>`"``."
    }
} else {
    $env:KB_DIRECTIVE_TEXT = ""
}
